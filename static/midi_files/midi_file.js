...
MidiEvent.createNote = function(note, sustained) {
    ...
    */
    notes = [];
    for (var i in note) {
        var n = {key: noteTable[note[i].key], duration: note[i].duration, volume: note[i].volume};
        notes.push(n);
    }

    var events = [];
    for (var i in notes) {
        events.push(MidiEvent.noteOn(notes[i].key, 0, notes[i].volume || DEFAULT_VOLUME));
    }
    if(!sustained) {
        events.push(MidiEvent.noteOff(notes[i].key, notes[i].duration || DEFAULT_DURATION, notes[i].volume || DEFAULT_VOLUME));
    }
    return events;
...
MidiEvent.noteOff = function(note, duration, volume) {
    return new MidiEvent({
        time:    note.duration || duration || 0, //UNCOMMENT
        type:    EVT_NOTE_OFF,
        channel: note.channel || DEFAULT_CHANNEL,
        param1:  note.pitch   || note,
        param2:  note.volume  || volume || DEFAULT_VOLUME
